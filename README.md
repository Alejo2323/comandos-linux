...
|/Lib|Este archivo se encarga de las bibliotecas más esenciales para ser ejecutadas de manera correcta por los binarios.|/lib/'machine-architecture|
|/Media|Es el punto de montaje con el que todos los volúmenes lógicos ya sean temporales como externas (memorias USB), entre otras particiones de disco.|/media/Usuario1|
|	/Opt|El Opt en algún modo podría ser básicamente una extensión del directorio al igual de todos aquellos archivos que son de para lectura y que de igual manera son parte de programas autocontenidos.|sudo chown root:root /opt|
...
|	/Sys|Es similar a / proc al cual contiene archivos que sean provenientes de la información del kernel y es relativa sobre eventos del SO (Ssistema Operativo).|ifconfig|
